...
app.post("/api/send-offer-email", async (req, res) => {
  ...
  try {
    ...
    const finalOfferText =
      typeof offerText === "string" && offerText.trim()
        ? offerText.trim()
  ...
  <div style="background: #ffffff; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
    ...
    try {
      if (MAIL_MODE === "sandbox") {
        ...
        console.log(`[offer-email] Sent via Mailtrap sandbox to ${recipientEmail}`, info?.messageId || "");
        return res.json({ ok: true });
      } else {
        const token = MAILTRAP_TOKEN || MAILTRAP_DEMO_TOKEN;
        const response = await fetch("https://send.api.mailtrap.io/api/send", {
          ...
          body: JSON.stringify({
            ...
            text: textContent,
            html: htmlContent,
            category: "offer-confirmation",
          }),
        });

        if (!response.ok) {
          const detail = await response.text();
          console.error("[offer-email] Mailtrap sending error:", detail);
          return res.status(500).json({
            error: "Mailtrap failed",
            detail,
          });
        ...
        console.log(`[offer-email] Sent via Mailtrap sending to ${recipientEmail}`);
        return res.json({ ok: true });
      }
    } catch (sendError) {
      const detail = sendError?.message || "Unknown Mailtrap error";
      console.error("[offer-email] Mailtrap error:", detail);
      return res.status(500).json({
        error: "Mailtrap failed",
        detail,
      });
